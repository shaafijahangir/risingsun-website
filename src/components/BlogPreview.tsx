      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
          {posts.map((post, i) => (
            ...
            >
              <Card className="overflow-hidden border-0 shadow-card hover:-translate-y-2 hover:shadow-lift transition-all duration-300 h-full">
        ...
        <div className={`text-center anim-fade-up ${isVisible ? "in-view" : ""}`}
             style={{ transitionDelay: "460ms" }}>
          <Link
            to="/blog"
            className="inline-flex items-center gap-2 text-brand-navy font-semibold hover:text-thai-gold transition-colors group"
          >
            {t("blog.readAll")} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
        </div>
      </div>
    </section>
